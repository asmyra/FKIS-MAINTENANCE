<?php
    ...
    if (isset($_POST['signup'])) {
       ...
       // $lname = mysqli_real_escape_string($mysqli, $_POST['lname']);
        ...
        $res = mysqli_query($mysqli, $sql) or die('query failed');
        ...
        // count erros
        if (count($errors) === 0) {
            $insertQuery = "INSERT INTO usersInfo (username,email,password,level,code,status)
            VALUES ('$username','$email','$password','$level',$code,'$status')";
            $insertInfo = mysqli_query($mysqli, $insertQuery);
    ...
    if (isset($_POST['verify'])) {
        ...
        $otp_result = mysqli_query($mysqli, $otp_query);
    ...
    if (isset($_POST['login'])) {
        ...
        $email_check = mysqli_query($mysqli, $emailQuery);

        if (mysqli_num_rows($email_check) > 0) {
            $passwordQuery = "SELECT * FROM usersInfo WHERE email = '$email' AND password = '$password'";
            $password_check = mysqli_query($mysqli, $passwordQuery);
